...
class Jacobi_block():
    def __init__(self,cfg):
        ...
        if cfg['physics_problem'] == 'heat_transfer':
            ...
            self.A_weights['D_matrix'] = tf.tile(tf.reshape(-8./3.*self.k,(1,1,1,1)),(1,110,110,1))
        else:
    ...
    def apply(self, f, max_itr=10):
        result = {}
        u_input = np.zeros((1, 110, 110, 1), 'float32')  # where u is unknown
...
if __name__ == "__main__":
    ...
    f = tf.placeholder(tf.float32,shape=(cfg['batch_size'], 110, 110, 1))
    u = tf.placeholder(tf.float32,shape=(cfg['batch_size'], 110, 110, 1))
    ...
    A1 = data1['K_forceboundary_nodes10x10']
    f1 = data2['f_forceboundary_elements10x10']
    x = data3['x0_elements10x10']

    #f1 = data['matrix'][0][0][1]
    #A1 = data['matrix'][0][0][0]
    u1 = np.linalg.solve(A1, f1)
    u_gt = u1.reshape(1,110,110,1)
    f1 = f1.reshape(1,110,110,1)
