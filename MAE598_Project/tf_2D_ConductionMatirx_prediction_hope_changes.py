...
def boundary_correct(x):
    bc_mask = np.ones((1,10,11,1))
    bc_mask[0,0,0,0] /= 2.
    bc_mask[0,0,-1,0] /= 2.
    bc_mask[0,-1,0,0] /= 2.
    bc_mask[0,-1,-1,0] /= 2.
    x *= bc_mask
    x = tf.pad(x[:, :, 1:-1, :], ((0,0), (0, 0), (1, 1), (0, 0)), "constant") # for boundary consideration
    return x
...
def conjgrad_tf(A_weights, b, x, n):
    ...
    A_dotx_conv = boundary_correct(A_dotx_conv)
...
if __name__ == '__main__':
    ...
    conductivity = tf.Variable(16., tf.float32)
    # Filter
